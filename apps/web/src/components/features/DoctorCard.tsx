        ...
        <CardContent className="flex-1">
          ...
          <div className="flex items-center mb-3">
            <div className="flex items-center space-x-1">
              ...
              <span className="font-semibold text-forest-900">{rating}</span>
              <span className="text-gray-600 text-sm font-medium">({reviewCount} reviews)</span>
            </div>
          </div>

          {/* Languages */}
          <div className="mb-3">
            <p className="text-xs text-gray-600 font-semibold mb-1">Languages:</p>
            <p className="text-sm text-forest-900 font-medium">{languages.join(', ')}</p>
          </div>

          {/* Location */}
          <div className="mb-3">
            <p className="text-xs text-gray-600 font-semibold mb-1">Location:</p>
            <p className="text-sm text-forest-900 font-medium">{city}</p>
          </div>
          ...
          {glowealPrice && (
            <div 
              onClick={(e) => {
                e.preventDefault();
                window.location.href = `/pricing/${glowealPrice.specialtySlug}`;
              }}
              className="block mt-3 cursor-pointer"
            >
              <div className="flex flex-wrap items-center gap-2 rounded-full bg-amber-50 border-2 border-amber-200 text-forest-700 px-4 py-2 text-sm font-semibold hover:bg-amber-100 transition-colors group/price">
